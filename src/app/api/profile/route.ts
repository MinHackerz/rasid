...
import { z } from 'zod';
import ct from 'country-to-currency';
...
export async function GET(request: NextRequest) {
    try {
        ...
        try {
            ...
            if (country) {
                const mappedCur = (ct as any)[country.toUpperCase()];
                if (mappedCur) {
                    detectedCurrency = mappedCur;
                }
            }
        } catch (e) {
            console.error('Failed to detect currency from headers:', e);
        }
        ...
        return NextResponse.json({
            success: true,
            data: { ...seller, logo, logoData: undefined, role: session.role || 'OWNER', detectedCurrency },
        });
    } catch (error) {
