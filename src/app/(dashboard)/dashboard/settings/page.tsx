...
import { PremiumSectionGuard } from '@/components/dashboard/PremiumSectionGuard';
            ...
            <div className="space-y-6">
                ...
                {activeTab === 'general' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                        ...
                        <PremiumSectionGuard
                            isAllowed={!!PLANS[(profile.plan || 'FREE') as PlanType]?.limits.emailIntegration}
                ...
                {/* INTEGRATIONS TAB */}
                {
                    activeTab === 'integrations' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                            <PremiumFeatureGuard
                                isAllowed={!!PLANS[(profile.plan || 'FREE') as PlanType]?.limits.emailIntegration}
                                featureName="Email & WhatsApp Integration"
                                description="Connect your own SMTP and WhatsApp Cloud API to send branded invoices."
                            >
                                <Card>
                                    <CardHeader
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                            <Card>
                                ...
                                <CardBody className="space-y-5">
                                    ...
                                    <PremiumFeatureGuard
                                        isAllowed={!!PLANS[(profile.plan || 'FREE') as PlanType]?.limits.emailIntegration}
                                        featureName="Auto-send Invoices"
                                        description="Automatically email or WhatsApp the invoice to the customer upon creation. Upgrade to unlock this feature."
                                        blurAmount="blur-[3px]"
                                    >
                                ...
                                <CardBody className="p-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        {INVOICE_TEMPLATES.map((template, index) => {
                                            ...
                                            const planLimits = PLANS[userPlan as PlanType]?.limits;

                                            // Check if template is available
                                            const isAvailable = planLimits?.templateIds.includes('all') ||
                                                planLimits?.templateIds.includes(template.id) ||
                                                (index < (planLimits?.templates || 1));
                                            ...
                                            return (
                                                <div key={template.id} className="relative">
                                                    <button
                                                        ...
                                                        className={`relative group rounded-2xl border-2 transition-all duration-300 text-left overflow-hidden w-full ${!isAvailable
                                                    ...
                                                    >
                                                        ...
                                                        <div className="aspect-[3/4] bg-neutral-50 overflow-hidden relative">
                                                            ...
                                                            <img
                                                                ...
                                                                className={`w-full h-full object-cover object-top ${!isAvailable ? 'filter blur-sm grayscale' : ''}`}
                                                            />
                                                    ...
                                                    {/* Upgrade Badge for Locked Templates */}
                                                    {!isAvailable && (
                                                        <div className="absolute top-3 right-3 z-10">
                                                            <div className="bg-neutral-900 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg flex items-center gap-1.5">
                ...
                {activeTab === 'team' && profile.role === 'OWNER' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                        <PremiumFeatureGuard
                            isAllowed={(PLANS[(profile.plan || 'FREE') as PlanType]?.limits.teamMembers || 1) > 1}
                            featureName="Team Management"
                            description={`Invite team members to collaborate. Your current plan allows ${PLANS[(profile.plan || 'FREE') as PlanType]?.limits.teamMembers || 1} team member${(PLANS[(profile.plan || 'FREE') as PlanType]?.limits.teamMembers || 1) > 1 ? 's' : ''}. Upgrade to invite more!`}
                            blurAmount="blur-[4px]"
                        >
                            <Card>
                                <CardHeader
